...
public class LogAspectOfPointCut {
     ...
     * 重点是看方法上的@Pointcut注解
     *
     * 下面的例子中用来匹配 public 任意返回值 basic.example.spring._ioc.test4.test1包下 任意类 任意方法 任意参数
     *
     ...
     *                                  对应下面的basic.example.spring._ioc.test4.test1.*, 表示basic.example.spring._ioc.test4.test1这个包下所有的类
     *                                  也支持basic.example.spring._ioc.test4.test1..*,(-> *前面可是.. <-) 表示basic.example.spring._ioc.test4.test1这个包及其子包中所有的类
     ...
     */
    @Pointcut("execution(public * basic.example.spring._ioc.test4.test1.*.*(..))")
    public void log() {
    ...
    // 要求被代理的目标类上面拥有basic.example.spring._ioc.test4.test1.pointcut.Test1 这个注解
    @Pointcut("@target(basic.example.spring._ioc.test4.test1.pointcut.Test1)")
    ...
    // 要求被代理的对象是Shop类或者其子类
    // +表示含Shop 的子类
    @Pointcut("target(basic.example.spring._ioc.test4.test1.pointcut.Shop2+)")
    ...
    // 要求被代理的方法上拥有basic.example.spring._ioc.test4.test1.pointcut.Test1 这个注解
    @Pointcut("@annotation(basic.example.spring._ioc.test4.test1.pointcut.Test1)")
